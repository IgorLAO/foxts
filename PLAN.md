...
> O foxcli (binário Go vendorizado em `foxcli/`) é a camada que fala com o VFP.
...
**Showcase "Catraca PCI" (FEITO — recria um app VFP real, modernizado):** prova de poder do framework —
recriar um app legado real (`GIT/pci/Pwi_VF9_CatracaPCI`, kiosk de catraca touch, ~26 telas, DLL serial, SQL
Server) com o UI Kit. Escopo decidido com o dono: **4 telas do fluxo principal, modernizadas** (não réplica
fiel): `SplashPage`←pci_inicio, `LoginPage`←system_logininicio, `PrincipalPage`←system_principal,
`ResultPage`←system_autorizou. Fluxo navegável `Splash→Login→Principal→Result` (FormManager.open+Release,
DataSession 2). Em `showcase/catraca-app/` — **projeto FoxTS padrão (empacotável em EXE)**: `vfp.config.json` (srcDir src,
outDir build, entry SplashPage) + `src/forms/*.form.tsx` + `src/components/Brand.tsx` + icons/ + build.sh/
report.js/run.prg/README. **EXE gerado e provado:** `node ../../vfp.js pack` → `build/catraca-app.exe` (+ .pjx +
manifest v6); os PNGs não entram no EXE (recursos de runtime) → copiar `icons/` ao lado do exe. `<Brand>`
reutilizável; Card/StatCard/FlatButton/Grid columns do kit; tema
azul em `vfp.theme.json`. Ícones herói grandes (Image do VFP não escala PNG alpha → rasterizar no tamanho exato
via `<Image src width height>`). As 4 telas compilam limpo (`sh build.sh` → 4 SCX) e foram **verificadas
visualmente** pelo `preview.js` (render headless fiel — ver fixes abaixo). Para ler os SCX legados criei um
leitor DBF+FPT minimo (`dist/_scxdump.js`, throwaway). **Achados/fixes no caminho (gerais, valem p/ todo o
framework):** (a) **`props={{...}}` agora funciona em TSX** (paridade com o caminho decorator — `readJsxAttrs`
chama `readPropsBag`; antes object-literal em atributo JSX caía no `emitExpr` e era rejeitado). Permite props VFP
cruas num controle JSX (ex.: `PasswordChar`, `Visible`). (b) **`preview.js` precarrega o Yoga** (igual ao
`foxc`): antes renderizava com o fallback flex onde `align/justify` são no-op → o preview MENTIA sobre
centralização (ícones saíam à esquerda); agora bate com o SCX real. (c) **`preview.js` respeita `Visible=.F.`**
(controle oculto não pinta) — antes a label de erro "Crachá inválido" aparecia sempre. (d) `@napi-rs/canvas`
adicionado como devDependency (o `preview.js` dependia dele e não estava instalado). **Pendente:** tudo no
...
- **Rede de regressão de output (FEITO 16/06):** `verifysnapshot.js` congela a IR (o que vira SCX) de cada `examples/*.form.tsx` em `snapshots/*.json` e falha se mudar — pega regressão SILENCIOSA do compilador (o resto da suíte só prova "ainda compila"). Serialização estável (chaves ordenadas → robusta a reorder inocente). 15 forms cobertos + 4 objeto-mode pulados (fora do path AST→IR). É a malha que torna seguro refatorar o monólito. `FOXTS_UPDATE_SNAPSHOTS=1` reescreve baselines após mudança intencional.
- **Dogfood de profundidade — lógica real da catraca (FEITO 16/06):** `examples/catraca.ts` modela a validação de acesso REAL do `Pwi_VF9_CatracaPCI` (regras NAO ENCONTRADO / SEM SALDO / LIBERADO + consumo de ingresso) em TS tipado → PRG, provada no VFP real (`verifycatraca.js`, **7/7**: cursor lookup via `count` E via `.first()` objeto-linha + UPDATE keyed). Surfou (e resolveu) gaps que só um app real expõe: (a) **cursor não tinha update** → adicionados `Cursor.update(col,val,whereCol,whereVal)` e `Cursor.increment(col,by,whereCol,whereVal)` → `UPDATE <cur> SET ... WHERE ...` (keyed, sem posicionar ponteiro; fox.ts + lowerCursorMethod); (b) **`row == null` / `!= null`** (resultado de `.first()`) agora compila p/ `ISNULL(row)` / `NOT ISNULL(row)` (emitBinary, antes de emitir os lados). **Gaps ainda anotados (backlog #1):** parâmetro com o MESMO nome de uma coluna vira `WHERE col = col` (sempre true) — precisa de guarda/aviso no compilador; `count()` não pode ser inline (tem que ser `const n = ...`); `.first()` exige `.orderBy()` (VFP: TOP requer ORDER BY) — podia ser auto-injetado.
- **Guarda parâmetro↔coluna (FEITO 17/06):** `where("col", col)` com o valor sendo um identificador de MESMO nome da coluna (→ `WHERE col = col`, sempre verdadeiro) agora é **rejeitado** com sugestão de renomear (`compiler/query.js`). Oráculo build-time `verifyqueryguard.js` (**2/2**: rejeita o footgun, aceita `colId`). Fecha o gap surfado no port da catraca.
- **Runner:** `test.js` agora **auto-descobre** `verify*.js`. `npm test` = **47/47** (oráculos `verify*.js` + build de todo `examples/*.form.ts(x)`; contagem cresce com a suíte — era 32/32 → 38/38 → hoje 44/44). Novos: `verifyguard.js` (guarda var 1-letra, build-time **2/2**) e `verifybinddefault.js` (default de bind por tipo, build+VFP **5/5**); `verifysql.js` ampliado com transações/SQLGETPROP; e o lote da composição React: `verifychildren.js`/`verifycompound.js`/`verifyicons.js`/`verifyhover.js`.
...
- `transpile.js` é monolítico (~2.8k linhas: parser+ir+jsx+emitter juntos). Modularizar em `compiler/{parser,ir,jsx,emitter}` (H4). **EM ANDAMENTO (16/06):** criada a `compiler/` e extraídos 5 módulos SOB A REDE DE SNAPSHOT (output byte-idêntico, snapshot 19/19, + oráculos VFP de cada frente passando): `compiler/util.js` (CompileError/ind/typeKind/isArrayType/foxString/dottedThisPath), `compiler/theme.js` (THEME + setTheme/themeColor/shade/hexToRGB — THEME é a MESMA referência mutada, então todos os `THEME.x` seguem válidos), `compiler/menu.js` (Frente G, auto-contido), `compiler/query.js` (Frente D), `compiler/validation.js` (Frente F, MESSAGES próprio), `compiler/style.js` (variant/color/class + ícones) e `compiler/jsx.js` (**o maior bloco — 1004 linhas**: `parseJsx` + leaves controlLeaf/containerLeaf/pageFrameLeaf/gridLeaf/flatButtonLeaf/sidebarItemLeaf/componentLeaf + helpers de AST JSX + toLayoutTree/applyConstructorDI/findRenderReturn/collectRoute). **7 módulos.** **Padrão p/ módulos acoplados:** o núcleo (`emitExpr`/`emitCall`) é injetado no `ctx` (só 2 sites de criação de ctx) → os módulos chamam `ctx.emitExpr` sem dependência circular. **transpile.js: 2881→1349 (−53%, monólito mais que pela metade).** Provado byte-idêntico a cada passo pela rede de snapshot. Resta no transpile.js: parser de statements/expr/classes + pipeline de form-IR (applyRuntimeColors/applyFlatChrome/finalizeFormIR/transpileForm) + entrada (loadProgram/compile). H4 essencialmente cumprida; refino opcional (separar emitter de IR-pipeline) fica p/ depois.
- ~~**Path do foxcli hardcoded**~~ ✅ **RESOLVIDO** e **foxcli VENDORIZADO no repo (15/06):** o foxcli (toolchain Go que fala com o VFP) era um binário num path-irmão "longe das coisas". Agora a **fonte Go vive em `foxcli/`** (`*.go`+`go.mod`, dependency-free) e o binário sai de `npm run foxcli:build` (`go build -C foxcli`); `npm run foxcli -- <args>` roda o bundlado (`foxcli.js` → `foxcli-path`). O `foxcli-path.js` resolve com precedência `FOXCLI` env > `FOXCLI_HOME` env > **`./foxcli/foxcli.exe` (BUNDLADO, fonte de verdade)** > `../foxcli/` (legado) > fallback hardcoded. `foxcli/foxcli.exe` é git-ignored (regerável). Pipeline self-contained: `npm run build` → foxc → foxcli (in-repo) → VFP. Todos os consumidores (`foxc.js`, `vfp.js`, `build.js`, `foxcli.js`, `verify*.js`) fazem `require('./foxcli-path')`.
- ~~**Oráculo + `@vfp/core`:** `require("@vfp/core")` não resolve em runtime; `foxc.build` tolera (pula oráculo).~~ ✅ **RESOLVIDO (15/06):** `foxc.js` instala um hook `Module._resolveFilename` (`aliasVfpCore`) que mapeia `@vfp/core` → o `decorators.ts` compilado (`dist/decorators.js`), e o `emitJs` passou a emitir CJS *carregável* p/ `.tsx` (`jsx: React`, factory irrelevante — o oráculo lê estrutura da AST, não roda `render()`). Agora forms que importam `@vfp/core` (decorator/objeto) **são cross-checados** em vez de pular o oráculo. Degrada só quando o top-level chama stubs compile-time (ex.: `schema(...)`/`str()` devolvem `null` por design) — igual antes.
...
## 🔎 Revisão DX 12/06/2026 — RECONCILIADA 15/06/2026 (quase tudo fechado)
...
**Status 15/06/2026 (conferido no código):** itens 1, 3, 4, 5, 6, 7, 8, 9 **✅ RESOLVIDOS**; item 2 **✅ RESOLVIDO** — `tsconfig.json` na raiz existe E o `loadProgram` agora o LÊ como fonte única (drift morto, subagente). Seção DX 12/06 fechada. Detalhe por item abaixo.

1. **[CORRIGIDO] `package.json` estava inválido** e quebrava o projeto INTEIRO: linha `"build:form": "` truncada (string não terminada, edição acidental). Não era só `npm run` — **qualquer `node *.js` morria** com `ERR_INVALID_PACKAGE_CONFIG` (o Node lê o package.json mais próximo p/ resolver `type`), inclusive `node test.js`. Restaurei o script a partir do README (linha 160: `foxc build examples/dias.form.ts -o dist/frmdiasts.scx`). Verificar se a intenção da edição era outra.
2. **Sem `tsconfig.json` na raiz do repo** — o build funciona porque `loadProgram` (transpile.js:1192) injeta as opções (jsx Preserve, experimentalDecorators, strict, paths `@vfp/core`→`decorators`) **por código**, mas o editor não as vê: abrir `examples/*.tsx` ou `showcase/*.tsx` no VS Code dá erro em todo JSX/decorator e `Cannot find module '@vfp/core'`; IntelliSense zero. É o 1º contato de qualquer dev com o repo. Fix barato: criar `tsconfig.json` na raiz espelhando as opções do `loadProgram` (include `examples`, `showcase`); idealmente o `loadProgram` passa a LER esse tsconfig (fonte única, sem drift). — ✅ **RESOLVIDO (15/06):** `tsconfig.json` na raiz existe E o `loadProgram` (transpile.js) o LÊ via `ts.readConfigFile`+`parseJsonConfigFileContent` (basePath = raiz do repo) como **fonte única** — jsx/strict/decorators/paths/target/lib derivam do tsconfig; só `noEmit`+resolução de módulo ficam no código; cópia hardcoded só como fallback à prova de crash. Drift morto.
3. **`vfp watch` ignora `.tsx`** — ✅ **RESOLVIDO** (vfp.js:456 `if (f && !/\.tsx?$/i.test(f)) return;`): `.ts` E `.tsx` disparam rebuild. Bônus também feitos: um 2º `fs.watch(root)` recompila ao mudar `vfp.(config|theme|messages).json` (vfp.js:467); alias `dev → vfp watch` no scaffold (vfp.js:149).
...
5. **Erros de tipo de arquivos importados são engolidos** — ✅ **RESOLVIDO** (transpile.js:~1268): o filtro de diagnósticos passou a excluir só libs do TS, `node_modules` e o `globals.d.ts` (ambiente) — agora reporta erros de tipagem de TODOS os arquivos do projeto, não só o entry. "Compilou = tipos ok" vale cross-file.
6. **`foxc.js` não trata `.tsx` ao derivar nomes** — ✅ **RESOLVIDO** (foxc.js:36 e :72 usam `path.basename(tsPath).replace(/\.tsx?$/, '')...`): o IR de um `.form.tsx` vira `dist/grid.json` (não `dist/grid.form.tsx.json`).
7. **Repo sem git** — ✅ **RESOLVIDO**: o repo está versionado (branch `ui-kit`, histórico com vários commits do UI Kit). `git diff`/branch/bisect disponíveis.
8. **Watch sem build incremental** (melhoria) — ✅ **RESOLVIDO** (vfp.js:427-470): o watch guarda o estado do último build completo (cache fonte→IR) e, quando muda UM `.form.ts(x)` já conhecido, refaz só o pré-passe de rotas (barato) + rebuilda só aquele form + regrava manifesto/routes. Qualquer dúvida/erro → build completo (conservador). Não derruba o watch em erro.
9. **`vfp build` não remove artefatos órfãos** — ✅ **RESOLVIDO** (vfp.js:~274): o build varre `dist/forms` e remove SCX cujo form não está no conjunto `keep` do build atual (preservando o `forms.manifest.json`). `dist/` volta a espelhar `src/`.
